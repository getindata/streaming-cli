...
class DeploymentAdapter:
    ...
    @staticmethod
    def deploy(deployment_yml: str, ververica_url: str, ververica_namespace: str, auth_token: str) -> Optional[str]: 
        deployments_url = f"{ververica_url}/api/v1/namespaces/{ververica_namespace}/deployments"
        response = DeploymentAdapter.post_deployment_file(deployments_url, auth_token, deployment_yml)
