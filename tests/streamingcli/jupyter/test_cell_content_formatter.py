...
class TestCellContentFormatter:
    variables_in_kernel_context = {
        "some_variable": 1,
        "some_table_name": "table_name"
    }

    """Test Jupyter cell input text enrichement with variables defined in IPython kernel"""
    def test_variable_enrichements(self):
        cell_input_text = "select * from {{ some_table_name }} v where v.id = { some_variable }"
        enriched_cell = CellContentFormatter(input_string=cell_input_text, user_ns=TestCellContentFormatter.variables_in_kernel_context).substitute_user_variables()
    ...
    def test_non_existent_variable_use(self):
        ...
        with pytest.raises(NonExistentVariableException):
            CellContentFormatter(input_string=cell_input_text, user_ns=TestCellContentFormatter.variables_in_kernel_context).substitute_user_variables()
